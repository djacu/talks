...
2. **Title:** Beholden to No One / The Nix Override Ladder / Nix Vegas 2026.

3. **Thesis.** Software makes decisions for you: forced updates, telemetry,
...
5. **One real module (nginx).** Purely to show the mechanism, no nginx-specific
   lesson. `ProtectSystem = "strict"` (plain, 100) vs `ProtectHome = mkDefault
   true` (1000). You override `ProtectHome` silently with a normal assignment;
   overriding `ProtectSystem` conflicts and needs `mkForce`. Use line-stepping.

6. **Firefox policies.** `programs.firefox.policies` repurposes enterprise
...
8. **Apply it yourself.** `overrideAttrs` + `fetchpatch` the unmerged PR. Code,
   line-stepping, shorten the URL so it does not clip. This is "overrule the
   package."

9. `[cut-first]` **Kernel, same lever at two altitudes.** `boot.kernelPatches`
...
10. **colors.js / faker.js sabotage (Jan 2022).** A maintainer intentionally
...
11. **Home Assistant declares you unsupported.** Not a corporation: owned by the
...
12. **`nixos-was-never-supported.patch`.** The whole nixpkgs reply is one line:
...
14. **Terraform goes BSL (Aug 2023).** HashiCorp relicenses overnight. nixpkgs
...
16. **xz backdoor (CVE-2024-3094).** nixpkgs briefly carried 5.6.1, but the
...
18. **The license fights the distro.** CC-BY-NC-ND forbids distributing modified
...
19. **But it is still yours.** A private one-line `postPatch` on your own machine
...
21. **Close.** Escape your fate. You have the source.
